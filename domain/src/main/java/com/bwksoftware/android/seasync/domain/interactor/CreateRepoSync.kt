 ...
 */
class CreateRepoSync @Inject
internal constructor(val repository: Repository, threadExecutor: ThreadExecutor,
                     postExecutionThread: PostExecutionThread) : UseCase<RepoTemplate, CreateRepoSync.Params>(
        threadExecutor, postExecutionThread) {

    override fun buildUseCaseObservable(params: Params): Observable<RepoTemplate> {
        return repository.syncRepo(params.authToken, params.repoID, params.storage)
    }

    class Params(val authToken: String, val repoID: String, val storage: String)
